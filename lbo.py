...
# NEW: _get_float and _get_int replace the bare float(input(...)) and int(input(...))
# calls in get_inputs() — same prompts, same behavior, but with a re-prompt loop so
# non-numeric entries, negatives, and out-of-range values don't crash the program
...
def get_inputs():
    ...
    purchase_price   = _get_float("Purchase Price ($M):             ")
 
    # ask how much of that came from the PE firm's own pocket
    # (the rest is assumed to be debt / borrowed money)
    equity_invested  = _get_float("Equity Invested ($M):            ")

    # NEW: equity must be less than purchase price — re-prompt if not (cross-input check)
    ...
    revenue          = _get_float("Starting Revenue ($M):           ")
 
    # EBITDA margin = what fraction of revenue turns into profit
    # e.g. 0.25 means 25 cents of every dollar of revenue is profit
    ebitda_margin    = _get_float("EBITDA Margin (e.g. 0.25):       ", min_val=0.001, max_val=0.999)
 
    # how fast do we expect revenue to grow each year?
    # e.g. 0.10 means 10% growth per year
    growth_rate      = _get_float("Revenue Growth Rate (e.g. 0.10): ", min_val=0.0, max_val=1.0, allow_zero=True)
 
    # what interest rate are we paying on our debt each year?
    interest_rate    = _get_float("Interest Rate (e.g. 0.08):       ", min_val=0.0, max_val=1.0, allow_zero=True)
    ...
    exit_multiple    = _get_float("Exit EV/EBITDA Multiple:         ")
 
    # how many years are we planning to hold this investment?
    holding_period   = _get_int(  "Holding Period (years):          ")
    ...
    tax_rate         = _get_float("Tax Rate (e.g. 0.25):            ", min_val=0.0, max_val=0.99, allow_zero=True)
    ...
    capex_pct        = _get_float("CapEx as % of Revenue (e.g. 0.03): ", min_val=0.0, max_val=0.99, allow_zero=True)
    ...
    annual_amort     = _get_float("Annual Debt Amortization ($M):   ", min_val=0.0, allow_zero=True)

    # NEW: cross-input check — margin + capex >= 100% means FCF will be negative before interest and taxes
    # a margin above 100% is technically a valid float but doesn't make financial sense
